...
| 28 | Pin `mempalace==3.3.0` (requirements.txt) désaligné du venv editable (3.3.5, fork `feat/configurable-embedder`) | ouvert sprint 12                |
...
## Note : signaux #28 et #29 révélés par le tour pin #27

Le tour pin #27 a révélé deux signaux de traçabilité venv hors
scope #27 (dettes #28 et #29 ci-dessus). #28 est non triviale :
un `pip install -r requirements.txt` réel rétrograderait mempalace
de 3.3.5 (fork editable) vers 3.3.0 et casserait le palace. À
arbitrer comme candidat sprint 13 — soit pin cohérent avec le venv
réel (3.3.5), soit référence editable au clone fork, soit
suppression du pin et bascule sur PyPI si le fork est mergé
upstream.
